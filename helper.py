...
def get_one_to_many_similarity_scores(to_compare: dict, compare_with: dict):
    ...
    for file in compare_with:
        ...
        similarity_scores[file] = round(score_matrix[0][1], 5)

    similarity_scores = sorted(similarity_scores.items(), key=lambda x: x[1], reverse=True)
    return similarity_scores[0]


def read_txt_file(data_path, file_name):
...
def load_text_files(data_path):
    txt_file_names = [i for i in os.listdir(data_path) if ".txt" in i]
    file_contents_dict = {}
    for file_name in tqdm(txt_file_names):
        file_contents_dict[file_name] = read_txt_file(data_path, file_name)
    return file_contents_dict
